...
async def update_forecast_cache():
    ...
    try:
        ...
        cache_metadata.update(
            {
                ...
                "next_update": (
                    end_time + timedelta(minutes=cache_metadata["update_interval_minutes"])
                ).isoformat(),
        ...
        print(
            ...
            f"Cache update completed: {successful_count} successful, "
...
async def lifespan(app: FastAPI):
    ...
    print(
        "Background forecast update task started "
        "(updates every 120 minutes - optimized for personal use)"
    )
...
app.add_middleware(
    TrustedHostMiddleware, allowed_hosts=["*"]  # TODO: Restrict to specific hosts in production
)
...
async def get_zones(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Unable to retrieve zones"
        )
    ...
    try:
        ...
        if "error" in cached_forecast:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail=(
                    f"Forecast for {zone_clean} currently unavailable: "
                    f"{cached_forecast['error']}"
                ),
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Unable to retrieve forecast"
        )
...
async def get_all_forecasts(request: Request):
    ...
    try:
        ...
        logger.info(
            f"Served all forecasts: {len(successful_forecasts)} successful, "
            f"{len(failed_zones)} failed"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Unable to retrieve forecasts"
        )
...
async def refresh_cache(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error refreshing cache"
        )
